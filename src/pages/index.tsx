...
export default () => {
  ...
  return (
    <>
      ...
      <MyComponent
        active={walletState === Web3Status.NoWallet}
        title={"Install Wallet"}
      />
      <MyComponent
        active={walletState === Web3Status.WalletExists}
        title={"Wallet Available"}
      />
      <MyComponent
        active={walletState === Web3Status.AccountConnected}
        title={`Account: ${address}`}
      />
      {walletState === Web3Status.NoWallet && <WalletOptions />}
      {walletState === Web3Status.WalletExists && (
        <button onClick={(action as WalletActionTypes).connect}>
          connect account
        </button>
      )}
    </>
