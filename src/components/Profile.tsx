...
import type { Profile as ProfileSchemaType, 
      ProfileRegistryDataType, 
      LocalizedString, 
      ProfileContract,
      ProfileRegistryCreateRequest } from '@/types';
...
export default function Profile() {
  ...
  const [currentProfile, setCurrentProfile] = useState<ProfileRegistryDataType>()
  ...
  const getLocalString = ( field: LocalizedString | undefined, locale:string ): string=> {
  ...
  const registerProfile = async (_profile: ProfileRegistryCreateRequest): Promise<string | null> => {
  ...
  useEffect(() => {
    ...
    if (contractState.profileRegisterError) {
      toast.error(`There was an error writing contract: ${contractState.profileRegisterError}` )
    }
          ...
          {Object.keys(profile).map((key) => (
            key !== 'image' ? (
              <div key={key}>
                ...
                <p className="mt-1 block w-full rounded-md p-2 pl-4">{profile[key as keyof Profile]}</p>
              </div>
            ) : null
          ))}
          {profile.image && (
